@extends("la.layouts.app")

@section("contentheader_title")
	<a href="{{ url(config('laraadmin.adminRoute') . '/agendamentos') }}">Agendamento</a> :
@endsection
@section("contentheader_description", $agendamento->$view_col)
@section("section", "Agendamentos")
@section("section_url", url(config('laraadmin.adminRoute') . '/agendamentos'))
@section("sub_section", "Edit")

@section("htmlheader_title", "Agendamentos Edit : ".$agendamento->$view_col)

@section("main-content")

@if (count($errors) > 0)
    <div class="alert alert-danger">
        <ul>
            @foreach ($errors->all() as $error)
                <li>{{ $error }}</li>
            @endforeach
        </ul>
    </div>
@endif

<div class="box">
	<div class="box-header">
		
	</div>
	<div class="box-body">
		<div class="row">
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($agendamento, ['route' => [config('laraadmin.adminRoute') . '.agendamentos.update', $agendamento->id ], 'method'=>'PUT', 'id' => 'agendamento-edit-form']) !!}
					@la_form($module)
					
					{{--
					@la_input($module, 'data')
					@la_input($module, 'aplicador')
					@la_input($module, 'paciente')
					--}}
                    <br>
					<div class="form-group">
						{!! Form::submit( 'Update', ['class'=>'btn btn-success']) !!} <button class="btn btn-default pull-right"><a href="{{ url(config('laraadmin.adminRoute') . '/agendamentos') }}">Cancel</a></button>
					</div>
				{!! Form::close() !!}
			</div>
		</div>
	</div>
</div>

@endsection

@push('scripts')
<script>
$(function () {
	$("#agendamento-edit-form").validate({
		
	});
});
</script>
@endpush
