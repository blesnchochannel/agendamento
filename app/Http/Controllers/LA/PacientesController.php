...
class PacientesController extends Controller
{
	...
	public $listing_cols = ['id', 'nome', 'cpf', 'email', 'data_nascimento', 'telefone', 'celular', 'responsavel', 'cep', 'rua', 'bairro', 'numero', 'cidade', 'estado', 'convenio', 'plano', 'inicio', 'local', 'codigo', 'indicacao', 'observacoes'];
	...
	public function dtajax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				...
				if(Module::hasAccess("Pacientes", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.pacientes.destroy', $data->data[$i][0]], 'method' => 'delete', 'id' => 'target'.$data->data[$i][0], 'style'=>'display:inline']);
